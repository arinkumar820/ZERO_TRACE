...
public class SnatchDetectionIntegration {
    ...
    public static void configureSensitivity(Context context, String sensitivity) {
        ...
        // Set high thresholds (only vigorous shaking triggers)
        switch (sensitivity.toLowerCase()) {
            case "low":
                editor.putFloat("jolt_threshold", 40.0f);  // Very high - extreme shaking only
                editor.putFloat("chaos_threshold", 15.0f);
                break;
            case "medium":
                editor.putFloat("jolt_threshold", 30.0f);  // High - vigorous shaking
                editor.putFloat("chaos_threshold", 10.0f);
                break;
            case "high":
                editor.putFloat("jolt_threshold", 25.0f);  // Moderate vigorous shaking
                editor.putFloat("chaos_threshold", 8.0f);
                break;
            default:
                Log.w(TAG, "Unknown sensitivity level, using medium");
                editor.putFloat("jolt_threshold", 30.0f);
                editor.putFloat("chaos_threshold", 10.0f);
        }
    ...
    public static void testSnatchDetection(Activity activity) {
        // Always allow testing for development
