    ...
    public void onBindViewHolder(@NonNull screen4RVAdaptor.screen4ViewHolder holder, int position) {
        ...
        holder.rl_chat.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                // Open ChatActivity with WebSocket integration
                Intent intent = new Intent(context, ChatActivity.class);
                intent.putExtra("chat_name", holder.name.getText().toString());
                intent.putExtra("chat_id", filteredList.get(holder.getAdapterPosition()).getId());
                intent.putExtra("receiver_id", filteredList.get(holder.getAdapterPosition()).getId());
