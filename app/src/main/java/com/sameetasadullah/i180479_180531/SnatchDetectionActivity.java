...
public class SnatchDetectionActivity extends AppCompatActivity implements SensorEventListener {
    ...
    // === HIGH THRESHOLD - ONLY VIGOROUS SHAKING ===
    private static final float JOLT_THRESHOLD = 30.0f;          // m/s² - High threshold for vigorous shaking
    private static final float CHAOS_THRESHOLD = 10.0f;         // rad/s - High threshold for vigorous rotation
    private static final long CHAOS_MONITORING_DURATION = 2500; // 2.5 seconds
    private static final int SENSOR_DELAY = SensorManager.SENSOR_DELAY_FASTEST; // Maximum sensitivity
    
    // Alternative detection methods (HIGH THRESHOLD)
    private static final float SINGLE_AXIS_THRESHOLD = 35.0f;   // Individual axis threshold
    private static final float GRAVITY_CHANGE_THRESHOLD = 25.0f; // Change in gravity
    private static final float EMERGENCY_AXIS_THRESHOLD = 40.0f; // Emergency detection
    private static final float TINY_MOVEMENT_THRESHOLD = 8.0f;   // High movement threshold
    ...
    private boolean sensorsStabilized = false;
    private long sensorStartTime = 0;
    private static final long SENSOR_STABILIZATION_PERIOD = 5000; // 5 seconds
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Add startup delay to prevent immediate triggers from device handling
        Handler startupHandler = new Handler(Looper.getMainLooper());
        startupHandler.postDelayed(() -> {
            Log.i(TAG, "🛡️ Starting snatch detection after startup delay...");
            startSnatchDetection();
            Log.i(TAG, "🛡️ SnatchDetection: Active monitoring started");
        }, 3000); // 3 second delay
        
        // Show startup message
        Toast.makeText(this, "🔒 Security initializing... Please wait", Toast.LENGTH_SHORT).show();
    }
    ...
    private void startSnatchDetection() {
        ...
        // Initialize sensor stabilization
        sensorStartTime = System.currentTimeMillis();
        sensorsStabilized = false;
        ...
        if (accelRegistered && gyroRegistered) {
            ...
            // Set timer for sensor stabilization
            Handler stabilizationHandler = new Handler(Looper.getMainLooper());
            stabilizationHandler.postDelayed(() -> {
                sensorsStabilized = true;
                Log.i(TAG, "🎯 Sensors stabilized - detection fully active");
                Toast.makeText(this, "🛡️ Security monitoring active", Toast.LENGTH_SHORT).show();
            }, SENSOR_STABILIZATION_PERIOD);
    ...
    private void handleAccelerometerData(SensorEvent event) {
        // Skip detection during sensor stabilization period
        if (!sensorsStabilized) {
            long timeSinceStart = System.currentTimeMillis() - sensorStartTime;
            if (timeSinceStart < SENSOR_STABILIZATION_PERIOD) {
                if (debugMode && timeSinceStart % 1000 < 100) { // Log every second during stabilization
                    Log.d(TAG, String.format("⏳ Sensors stabilizing... %d/%d seconds", 
                        timeSinceStart/1000, SENSOR_STABILIZATION_PERIOD/1000));
                }
                return; // Skip all detection during stabilization
            }
        }
        ...
        // Trigger 5: Detect vigorous shaking movement (high threshold)
        if (Math.abs(x) > 20.0f || Math.abs(y) > 20.0f || Math.abs(z) > 20.0f) {
            joltTriggered = true;
            triggerReason = "Vigorous shaking movement detected";
        }
        
        // Trigger 6: Combined axis movement (high threshold)
        float totalAxisMovement = Math.abs(x) + Math.abs(y) + Math.abs(z);
        if (totalAxisMovement > 50.0f) {
            joltTriggered = true;
            triggerReason = "Combined vigorous axis movement";
        }
    ...
    private void handleGyroscopeData(SensorEvent event) {
        ...
        // Trigger 4: Any single axis above vigorous shaking threshold
        if (Math.abs(x) > 8.0f || Math.abs(y) > 8.0f || Math.abs(z) > 8.0f) {
            chaosTriggered = true;
            chaosReason = "Single axis vigorous rotation";
        }
        
        // Trigger 5: High movement threshold for vigorous shaking
        if (Math.abs(x) > TINY_MOVEMENT_THRESHOLD || Math.abs(y) > TINY_MOVEMENT_THRESHOLD || Math.abs(z) > TINY_MOVEMENT_THRESHOLD) {
            chaosTriggered = true;
            chaosReason = "Vigorous movement detected";
        }
        
        // Trigger 6: Multi-axis vigorous movement (high sensitivity)
        if ((Math.abs(x) > 6.0f && Math.abs(y) > 6.0f) || 
            (Math.abs(y) > 6.0f && Math.abs(z) > 6.0f) || 
            (Math.abs(x) > 6.0f && Math.abs(z) > 6.0f)) {
            chaosTriggered = true;
            chaosReason = "Multi-axis vigorous movement";
        }
        
        // Trigger 7: Combined vigorous movements (high threshold)
        if (combinedMovement > 12.0f) {
            chaosTriggered = true;
            chaosReason = "Combined vigorous movement";
        }
